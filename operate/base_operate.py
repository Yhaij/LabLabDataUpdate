...
def connection_sql(db, host='10.1.13.29', user='root', passwd='tdlabDatabase', port=3306, charset='utf8'):
    """
    连接数据库 
    """
    print '连接数据库 %s ...' % db
...
def select_execute(cur, sql_str):
    return cur.execute(sql_str)


def add_sql(cur, table, **kwargs):
    """
    数据库插入
    :param cur: 
...
def select_sql(cur, table, *args, **kwargs):
    """
    数据库查询
    :param cur: 
    :param table: 查询的表单
...
def update_sql(cur, table, where=None, **kwargs):
    """
    数据库更新字段
    :param cur: 
    :param table: 更新的表单
